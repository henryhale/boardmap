...
export class HeatmapCalculator {
	static calculateInfluenceHeatmap<T extends BoardEntry>(gameEngine: IGameEngine<T>) {
	...
	static calculateMovementHeatmap<T extends BoardEntry>(gameEngine: IGameEngine<T>) {
		const heatmap = createBlankGrid(gameEngine.boardSize);
	...
	static calculateThreatHeatmap<T extends BoardEntry>(gameEngine: IGameEngine<T>) {
		const heatmap = createBlankGrid(gameEngine.boardSize);
			...
			for (let col = 0; col < gameEngine.boardSize; col++) {
				...
				if (piece && piece.player !== currentPlayer) {
					...
					moves.forEach(([moveRow, moveCol]) => {
						...
						if (gameEngine.board[moveRow][moveCol] && gameEngine.board[moveRow][moveCol].player === currentPlayer) {
